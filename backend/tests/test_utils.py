...
class TestExplainView:
    ...
    def test_missing_required_fields(self, auth_client, db):
        response = auth_client.post(self.url, {'sk_id_curr': 100001})
        assert response.status_code == 400

    @patch('api.views.get_shap_values')
    @patch('api.views.get_default_model_id')
    def test_success(self, mock_default_id, mock_shap, auth_client, ml_model, client_feature):
        ...
        assert 'shap_values' in response.data

    @patch('api.views.get_shap_values')
    @patch('api.views.get_default_model_id')
    def test_client_not_found_raises_404(self, mock_default_id, mock_shap, auth_client, db, ml_model):
        mock_default_id.return_value = ml_model.id
        mock_shap.side_effect = ValueError('Client not found in database')
        payload = {
            'sk_id_curr': 999999,
            'amt_income': 100000,
            'amt_credit': 300000,
    ...
    @patch('api.views.get_shap_values')
    @patch('api.views.get_default_model_id')
    def test_shap_server_error(self, mock_default_id, mock_shap, auth_client, db, ml_model):
        mock_default_id.return_value = ml_model.id
        mock_shap.side_effect = RuntimeError('Model crashed')
        payload = {
            'sk_id_curr': 100001,
            'amt_income': 100000,
            'amt_credit': 300000,
    ...
    def test_explain_unauthenticated(self, api_client, db):
